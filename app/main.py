...
class CarWashStation:
    ...
    def serve_cars(self, cars: list, income: float = 0) -> float:
        for car in cars:
    ...
    def calculate_washing_price(self, car: callable) -> float:
        calc_price = (car.comfort_class * (self.clean_power - car.clean_mark)
    ...
    def rate_service(self, rate: int) -> None:
        avg_rating, count_ratings = self.average_rating, self.count_of_ratings
        self.average_rating = round((avg_rating * count_ratings + rate)
                                    / (count_ratings + 1), 1)
