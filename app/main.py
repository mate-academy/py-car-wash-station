...
class CarWashStation:
    ...
    def serve_cars(self, cars: list) -> float:
        ...
        for car in cars:
            if car.clean_mark < self.clean_power:
                summa += self.calculate_washing_price(car)
                self.wash_single_car(car)
        return round(summa, 1)

    def calculate_washing_price(self, cars: Car) -> float:
        return (
            round(cars.comfort_class * (self.clean_power - cars.clean_mark)
    ...
    def rate_service(self, digit: int) -> None:
        self.average_rating = round((self.average_rating
                                     * self.count_of_ratings + digit)
