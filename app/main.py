...
class Car:
    def __init__(self,
        ...
        if not 1 <= comfort_class <= 7:
            raise ValueError(
                "comfort_class must be between 1 and 7 inclusive."
            )
...
class CarWashStation:
    def __init__(self, distance_from_city_center: float,
        ...
        if not 1 <= distance_from_city_center <= 7:
            raise ValueError(
                "comfort_class must be between 1.0 and 10.0 inclusive."
            )
    ...
    def rate_service(self, user_rating: int) -> None:
        ...
        average_rating = ((self.average_rating * (self.count_of_ratings - 1)
                           + user_rating) / self.count_of_ratings)
        self.average_rating = round(average_rating, 1)
