...
class CarWashStation:
    ...
    def calculate_washing_price(self, car_to_wash: Car) -> float:
        washing_price = round(car_to_wash.comfort_class *
                              (self.clean_power - car_to_wash.clean_mark) *
                              self.average_rating / self.distance_from_city_center, 1)
        return washing_price

    def wash_single_car(self, car_to_wash: Car) -> bool:
        if self.clean_power > car_to_wash.clean_mark:
    ...
    def rate_service(self, rate_point: float) -> None:
        self.average_rating = round((self.average_rating * self.count_of_ratings + rate_point) /
